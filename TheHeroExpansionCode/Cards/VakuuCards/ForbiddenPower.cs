...
public class ForbiddenPower() : VakuuCard(0, CardType.Skill, TargetType.Self)
{
    ...
    protected override async Task OnPlay(
        ...
        CardPlay cardPlay)
    {
        ForbiddenPower forbiddenPower = this;

        foreach (var card in PileType.Hand.GetPile(forbiddenPower.Owner).Cards.ToList())
        {
            if (card.Enchantment == null && card.Type == CardType.Attack)
                if (forbiddenPower.IsUpgraded)
                {
